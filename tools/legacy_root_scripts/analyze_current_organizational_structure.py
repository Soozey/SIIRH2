...
Ce script examine :
- Les tables employers et workers existantes
- Les colonnes organisationnelles actuelles (etablissements, departements, services, unites)
- Les patterns d'utilisation dans les données existantes
"""
...
try:
    ...
    from siirh_backend.app.config.config import DATABASE_URL
    from siirh_backend.app.models import Employer, Worker
except ImportError:
    # Fallback si les imports ne fonctionnent pas
    print("⚠️  Impossible d'importer les modules backend")
    print("Exécution en mode simulation avec données fictives")
    DATABASE_URL = None
...
def analyze_organizational_structure():
    ...
    if DATABASE_URL is None:
        ...
        print("Vérifiez que le backend est configuré correctement")
        return None
